...
function evision_corporate_setup() {
     ...
     */
    add_image_size( 'corporate-main-banner', 1370,490, true );
...
function evision_corporate_scripts() {
    ...
    if( isset( $evision_corporate_customizer_all_values['evision-corporate-map-enable-plugin']) && !empty($evision_corporate_customizer_all_values['evision-corporate-map-enable-plugin'] )){
        $evision_corporate_map_plugin = $evision_corporate_customizer_all_values['evision-corporate-map-enable-plugin'];
    }
    else{
        $evision_corporate_map_plugin = 0;
    }
    ...
    $translation_array = array(
        ...
        'evision_corporate_map_plugin' => $evision_corporate_map_plugin,
...
/*Enque script for admin panel*/
function evision_corporate_admin_scripts() {
    wp_enqueue_script( 'evision-corporate-panel-js', get_template_directory_uri() . '/assets/js/admin-panel.js', array(), '' );
    wp_enqueue_style( 'evision-corporate-admin-section-css', get_template_directory_uri() . '/assets/css/admin-section.css', array(), '' );
    }
add_action( 'admin_enqueue_scripts', 'evision_corporate_admin_scripts' );
...
/*update to pro added*/
require_once( trailingslashit( get_template_directory() ) . 'trt-customize-pro/evision-corporate/class-customize.php' );